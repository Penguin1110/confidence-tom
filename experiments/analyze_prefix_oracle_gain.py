...
def _load_tasks(cfg: DictConfig) -> dict[str, object]:
...
def main(cfg: DictConfig) -> None:
    ...
    for file_path in files:
        ...
        for row in rows:
            ...
            steps = row.get("prefix_oracle_steps", [])
            ...
            for idx, step in enumerate(steps):
                ...
                current_segment_text = prefix_segments[-1]["text"] if prefix_segments else ""
                prev_segment_text = prefix_segments[-2]["text"] if len(prefix_segments) >= 2 else ""
    ...
    summary["full_trace_correct_tasks"] = summary["full_trace_correct"]["count"]
    summary["positive"] = summary["delta_buckets"]["positive"]
    summary["zero"] = summary["delta_buckets"]["zero"]
    summary["negative"] = summary["delta_buckets"]["negative"]
    summary["tasks_with_any_positive_gain"] = summary["tasks_with_positive_gain"]["count"]
    summary["tasks_with_any_negative_gain"] = summary["tasks_with_negative_gain"]["count"]
    ...
    print("\n== Avg Delta vs Step ==")
    for step, value in avg_delta_vs_step.items():
        counts = per_step_counts[step]
        print(
            f"step {step}: avg_delta={value:.3f}"
