...
def _load() -> tuple[list[dict[str, object]], np.ndarray]:
    rows = [
        json.loads(line)
...
def _split(rows: list[dict[str, object]], emb: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
...
def _probe_binary(
    rows: list[dict[str, object]], emb: np.ndarray, target_key: str, positive_value: object
) -> dict[str, object]:
...
def _pca_2d(emb: np.ndarray) -> dict[str, object]:
...
def main() -> None:
    ...
    meta = json.loads((DATA_DIR / "pilot_meta.json").read_text(encoding="utf-8"))
    pca = _pca_2d(emb)
    report = {
        "rows": len(rows),
