...
OUTPUT_DIR = RESULTS_DIR / "_early_decision_v1"
OUTPUT_CSV = OUTPUT_DIR / "early_decision_rows.csv"
...
def _extract_text_features(prefix_text: str) -> dict[str, float]:
    ...
    _, certainty_density = _pattern_density(prefix_text, _CERTAINTY_PATTERNS)
    return {
...
def _load_task_index(run_name: str) -> dict[str, dict[str, object]]:
    ...
    for task in data:
        index[str(task["task_id"])] = {
            ...
            "prefix_text_index": {
                str(step.get("prefix_id", "")): str(step.get("prefix_text", ""))
...
def _step_bucket(step_index: int) -> str:
    if step_index <= 1:
        return "1"
    if step_index == 2:
        return "2"
...
def main() -> None:
    ...
    for spec in RUN_SPECS:
        ...
        with csv_path.open(newline="", encoding="utf-8") as f:
            ...
            for raw in reader:
                task_id = raw["task_id"]
                task_meta = task_index[task_id]
                if task_id not in task_ids_seen:
                    task_ids_seen.add(task_id)
                    success_tasks += int(task_meta["full_trace_correct"])
                step_index = int(_to_float(raw["step_index"]))
                prefix_id = str(raw["prefix_id"])
                prefix_text = str(task_meta["prefix_text_index"].get(prefix_id, ""))
                row: dict[str, object] = {
                    ...
                    "task_id": task_id,
                    "prefix_id": prefix_id,
                    "small_full_trace_success": int(task_meta["full_trace_correct"]),
                    "step_bucket": _step_bucket(step_index),
                }
        ...
        per_run_tasks[spec.run_name] = len(task_ids_seen)
        per_run_success_tasks[spec.run_name] = success_tasks
    ...
    meta = {
        ...
        "step_buckets": ["1", "2", "3", "4+"],
        "notes": [
            "Rows come from per_prefix_rows.csv across OlympiadBench and LiveBench family runs.",
            "Target is whether the small model's full trace for that task ultimately succeeds.",
            "This is an early-decision dataset: each prefix row inherits the task-level small full-trace label.",
            "Only prefix-observable features are kept in the training dataset.",
        ],
        "per_run_counts": per_run_counts,
        "per_run_tasks": per_run_tasks,
        "per_run_success_tasks": per_run_success_tasks,
    }
