...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Launch remote Mistral-local family sweep jobs on the company machine."
    )
    ...
    if args.target in {"olympiad", "both"}:
        jobs.append(("mistral_local_olympiad", "prefix_family_sweep_mistral_local"))
    if args.target in {"livebench", "both"}:
        jobs.append(("mistral_local_livebench", "prefix_family_sweep_livebench_mistral_local"))
