...
async def _evaluate_task(
    ...
    if handoff_step is not None and feature_history:
        ...
        continue_cost_est = (
            ...
            else projected_small_continue_tokens / 1000.0 * small_pricing.output_per_1k
        )
...
@hydra.main(version_base=None, config_path="../configs", config_name="intervention_pilot")
def main(cfg: DictConfig) -> None:
    ...
    for large_model in cfg.large_worker.models:
        out_path = (
            output_dir
            / f"{_sanitize_label(str(cfg.small_worker.label))}_to_{_sanitize_label(str(large_model))}.json"
        )
