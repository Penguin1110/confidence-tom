...
def _load_rows() -> list[dict[str, str]]:
    if not DATASET_CSV.exists():
        raise FileNotFoundError(
            f"Missing dataset CSV: {DATASET_CSV}. Run build_early_decision_dataset.py first."
        )
...
def _metrics_by_step_bucket(
    test_rows: list[dict[str, str]], y_true: np.ndarray, probs: np.ndarray, threshold: float
) -> dict[str, dict[str, float]]:
    buckets = ["1", "2", "3", "4+"]
    out: dict[str, dict[str, float]] = {}
    for bucket in buckets:
        idx = [i for i, row in enumerate(test_rows) if row.get("step_bucket") == bucket]
        if not idx:
            continue
        y_b = y_true[idx]
        p_b = probs[idx]
        out[bucket] = {
            "count": int(len(idx)),
            **_classification_report(y_b, p_b, threshold=threshold),
        }
    return out
...
def _run_experiment(
    ...
    *,
    ...
    return {
        ...
        "test_metrics_by_step_bucket": _metrics_by_step_bucket(
            split.test_rows, split.y_test, test_probs, threshold
        ),
...
def main() -> None:
    ...
    experiments = [
        ...
        _run_experiment(
            rows, name="state_signals", numeric_features=STATE_FEATURES, include_family_onehot=False
        ),
        _run_experiment(
            rows,
    ...
    summary = {
        ...
        "notes": [
            "This is the first Early Decision baseline on prefix-observable features only.",
            "It predicts whether the small model's full trace for the task eventually succeeds.",
            "Each prefix row inherits the task-level small full-trace label.",
        ],
    ...
    for exp in experiments:
        ...
        print(
            f"{exp['name']}: auroc={test['auroc']:.3f} f1={test['f1']:.3f} precision={test['precision']:.3f} recall={test['recall']:.3f}"
        )
