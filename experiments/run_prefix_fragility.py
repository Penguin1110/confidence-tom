...
Return only the rewritten prefix. If you cannot safely rewrite it under these constraints, return the original prefix unchanged."""
...
def _extract_final_answer(text: str) -> str:
    ...
    if not lines:
        return ""
    last = lines[-1]
    return last if re.search(r"(sqrt|\\sqrt|\\boxed|boxed|[0-9]|yes|no)", last, flags=re.I) else ""
...
def _load_task_map(benchmark: str) -> dict[str, object]:
    ...
    messages = [
        {"role": "system", "content": _REWRITE_SYSTEM},
        {
            ...
            "content": (
                ...
                "Rewrite the prefix so it says exactly the same thing, with the same level of certainty, "
                "but with slightly cleaner surface wording. If there is any risk of changing meaning, "
...
async def _continue_from_prefix(client: LLMClient, question: str, prefix_text: str) -> str:
    ...
    return await client.agenerate_text(messages, max_tokens=CONTINUE_MAX_TOKENS, temperature=0.0)
...
def _group_by(
    rows: list[dict[str, object]], key: str, family_only: bool = False
...
def _to_markdown(summary: dict[str, object]) -> str:
    lines = [
        ...
        "",
        f"- rows: `{summary['rows']}`",
        f"- normalized correctness changed rate: `{summary['normalized_correctness_changed_rate']:.3f}`",
        f"- rewritten correctness changed rate: `{summary['rewritten_correctness_changed_rate']:.3f}`",
