...
_SMALL_CONTINUE_SYSTEM_PROMPT = """You are the same small worker continuing an existing reasoning prefix.
Continue naturally from the given prefix and finish the problem.
Do not restart from scratch unless the prefix is unusable.
Do not restate the full problem or rebuild the derivation from the beginning if the prefix already contains useful structure.
Do not use JSON.
End with a final line exactly in the form:
Final Answer: <answer>
"""

_LARGE_TAKEOVER_SYSTEM_PROMPT = """You are a stronger reasoning assistant taking over from a partial reasoning prefix.
...
def _pricing_from_cfg(cfg: DictConfig, model_name: str) -> Optional[ModelPricing]:
    ...
    if pricing.input_per_1k == 0.0 and pricing.output_per_1k == 0.0 and pricing.reasoning_per_1k == 0.0:
        return None
    return pricing
...
async def _extract_answer_with_fallback(raw_text: str, extract_client: Optional[LLMClient]) -> str:
    ...
    if extract_client is not None:
        parsed, _ = await parse_with_llm_fallback(raw_text, ExtractedFinalAnswerOutput, extract_client)
...
def _clean_segments(segments: list[PrefixSegment]) -> list[PrefixSegment]:
    ...
    def _is_header_only(text: str) -> bool:
        stripped = text.strip()
        return bool(re.fullmatch(r"(?:#{1,6}\s+.+|Step\s+\d+\s*:?.+)", stripped, flags=re.IGNORECASE))
    ...
    merged: list[str] = []
    for text in cleaned:
        if merged and (_is_header_only(text) or _is_short_transition(text) or _is_formula_block(text)):
            merged[-1] = f"{merged[-1]}\n{text}".strip()
            continue
        if (
            merged
...
async def _generate_text(
    ...
    for attempt in range(1, attempts + 1):
        try:
            ...
            logger.info("%s.raw task=%s raw_len=%d raw_head=%r", tag, task_id, len(raw or ""), (raw or "")[:200])
            return raw, trace
...
async def _map_task(task: StaticTask, cfg: DictConfig) -> PrefixOracleGainTaskResult:
    ...
    if resume_payload and resume_payload.get("status") in {"full_trace_done", "prefix_step_done"}:
        ...
        segments, parsed_final_answer, parse_incomplete = await _segment_full_trace(full_text, extract_client, task.id)
        ...
        partial_store.save(
            task.id,
            {
                ...
                "full_trace_api_trace": full_trace_api.model_dump() if full_trace_api is not None else None,
    ...
    for step_index in range(start_step_index, len(segments) + 1):
        ...
        try:
            ...
            logger.error("large_takeover_prefix.error task=%s step=%d\n%s", task.id, step_index, traceback.format_exc())
...
@hydra.main(version_base=None, config_path="../configs", config_name="prefix_oracle_gain_mapping")
def main(cfg: DictConfig) -> None:
    ...
    if benchmark_name == "olympiadbench":
        ...
        livebench_count = int(cfg.dataset.get("livebench_reasoning", cfg.dataset.get("livebench", 0)))
    ...
    out_path = output_dir / f"{_sanitize_label(str(cfg.small_worker.label))}_to_{_sanitize_label(str(cfg.large_worker.label))}.json"
