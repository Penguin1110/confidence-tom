...
OUT_MD = ROOT / "docs" / "prefix_task_structure_analysis.md"
...
def _load_summaries() -> dict[str, dict]:
    ...
    for run_name in RUN_NAMES:
        ...
        out[run_name] = json.loads(path.read_text())
    return out


def _task_feature_rows(summaries: dict[str, dict]) -> list[dict[str, object]]:
    all_task_ids = set()
    for summary in summaries.values():
        all_task_ids.update(summary["per_task_summary"].keys())
    ...
    for task_id in sorted(all_task_ids):
        ...
        feature_row: dict[str, object] = {"task_id": task_id}

        for run_name in RUN_NAMES:
            summary = summaries[run_name]
            task_summary = summary["per_task_summary"][task_id]
...
def _build_clusters(rows: list[dict[str, object]], k: int = 4) -> dict[str, object]:
    feature_names = [
...
def _to_markdown(map_rows: list[dict[str, object]], clustering: dict[str, object]) -> str:
    lines: list[str] = []
    ...
    lines.append("")
    for cluster in sorted(clustering["clusters"], key=lambda item: item["cluster_id"]):
        lines.append(
