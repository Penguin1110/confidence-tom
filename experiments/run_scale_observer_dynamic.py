...
from confidence_tom.observer.dynamic_models import JudgmentOutput
...
@hydra.main(version_base="1.3", config_path="../configs", config_name="scale_dynamic")
def main(cfg: DictConfig) -> None:
    asyncio.run(amain(cfg))


async def amain(cfg: DictConfig) -> None:
    observer_output_dir = Path(getattr(cfg, "observer_output_dir", f"{cfg.output_dir}_observer"))
    checkpoint = ObserverCheckpointManager(observer_output_dir)
    ...
    logger.info(f"Observer results saved to: {observer_output_dir}/")
