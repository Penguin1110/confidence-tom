...
def _group_stats(records: list[TaskRecord]) -> dict[str, object]:
    ...
    return {
        ...
        "taxonomy_definition": {
            "stable_success": f"full_correct and first_correct_frac <= {EARLY_FRACTION:.3f} and local_correct_rate >= {MIN_STABLE_LOCAL_RATE:.3f}",
...
def _render_markdown(summary: dict[str, object], records: list[TaskRecord]) -> str:
    ...
    for cat in ["stable-success", "late-success", "fragile-success", "persistent-failure"]:
        ...
        lines.append(
            f"| {cat} | {stats['n']} | {stats['n'] / total:.3f} | {stats['mean_first_correct_frac']:.3f} | {stats['mean_local_correct_rate']:.3f} |"
        )
    lines.append("")
    ...
    lines.append("| --- | ---: | ---: | ---: | ---: | ---: |")
    for k in ["True", "False"]:
        block = summary["direct_vs_reentry"][k]
        lines.append(
            f"| {k} | {block['n']} | {block['any_small_rate']:.3f} | {block['last_small_rate']:.3f} | {block['mean_first_correct_frac']:.3f} | {block['mean_local_correct_rate']:.3f} |"
        )
    lines.append("")
    lines.append("## Transition Matrix")
    lines.append("")
    tm = summary["transition_matrix"]
    lines.append(f"- full correct & any small correct: `{tm['full_and_any_small_correct']}`")
    lines.append(f"- full correct & no small correct: `{tm['full_and_no_small_correct']}`")
    lines.append(f"- full wrong & any small correct: `{tm['wrong_and_any_small_correct']}`")
    lines.append(f"- full wrong & no small correct: `{tm['wrong_and_no_small_correct']}`")
    lines.append("")
    ...
    for bench, stats in sorted(summary["by_benchmark"].items()):
        ...
        lines.append(
            f"| {bench} | {counts['stable-success']} | {counts['late-success']} | {counts['fragile-success']} | {counts['persistent-failure']} | {stats['mean_first_correct_frac']:.3f} |"
        )
    lines.append("")
    ...
    for family, stats in sorted(summary["by_family"].items()):
        ...
        counts = Counter(r.category for r in block)
        lines.append(
            f"| {family} | {counts['stable-success']} | {counts['late-success']} | {counts['fragile-success']} | {counts['persistent-failure']} | {stats['mean_first_correct_frac']:.3f} |"
        )
    lines.append("")
    lines.append("## Notes")
    lines.append("")
