...
async def _map_task(task: StaticTask, cfg: DictConfig) -> OracleGainTaskResult:
    ...
    for step_index in range(1, len(base_trace.steps) + 1):
        ...
        small_score = float(small_continue_eval.score or 0.0)
        large_score = float(large_takeover_eval.score or 0.0)
        delta_correctness = large_score - small_score
...
@hydra.main(
    version_base=None,
    config_path="../../../../configs",
    config_name="oracle_gain_mapping",
)
