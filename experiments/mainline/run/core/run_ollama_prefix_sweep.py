...
class LargeWorkerSpec(TypedDict):
    family: str
    model: str
    label: str
    max_tokens: int
...
class SmallDefaultSpec(TypedDict):
    model: str
    label: str
    max_tokens: int


LARGE_WORKERS: list[LargeWorkerSpec] = [
    {
        "family": "openai",
...
SMALL_DEFAULTS: dict[str, SmallDefaultSpec] = {
    "qwen": {
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Run prefix takeover sweep with small model via Ollama and large model via OpenRouter."
        )
    )
