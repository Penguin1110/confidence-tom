...
RUNS = {
    "qwen_to_openai_50": "outputs/results/qwen_to_openai_50/Qwen_3_14B_to_GPT_5_4.json",
    (
        "qwen_to_anthropic_50"
    ): "outputs/results/qwen_to_anthropic_50/Qwen_3_14B_to_Claude_Opus_4_6.json",
    "llama_to_openai_50": "outputs/results/llama_to_openai_50/Llama_4_Scout_to_GPT_5_4.json",
    (
        "llama_to_anthropic_50"
    ): "outputs/results/llama_to_anthropic_50/Llama_4_Scout_to_Claude_Opus_4_6.json",
    (
        "mistral_to_openai_50"
    ): "outputs/results/mistral_to_openai_50/Ministral_8B_2512_to_GPT_5_4.json",
    (
        "mistral_to_anthropic_50"
    ): "outputs/results/mistral_to_anthropic_50/Ministral_8B_2512_to_Claude_Opus_4_6.json",
}


def build_summary(data: list[dict[str, Any]]) -> dict[str, Any]:
    tasks = len(data)
    ...
    per_step: defaultdict[int, list[int]] = defaultdict(list)
    per_step_pos: defaultdict[int, int] = defaultdict(int)
    per_step_neg: defaultdict[int, int] = defaultdict(int)
    zero_both_correct = 0
    zero_both_wrong = 0
    zero_mixed = 0
    per_task_summary: dict[str, dict[str, int]] = {}
    for row in data:
        any_pos = False
        any_neg = False
        tpos = tzero = tneg = 0
