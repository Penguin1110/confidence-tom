...
def _build_matrix(
    rows: list[Row],
) -> tuple[list[Row], np.ndarray, np.ndarray, np.ndarray, np.ndarray, list[str]]:
    train_rows: list[Row] = []
    test_rows: list[Row] = []
    for row in rows:
...
def _probability_stable_msp(
    sorted_rows: list[dict[str, Any]], label: int, threshold: float
) -> int | None:
...
def _summarize(task_groups: list[TaskGroup], key: str) -> dict[str, Any]:
...
def _slice_summary(task_groups: list[TaskGroup], key: str, field: str) -> dict[str, dict[str, Any]]:
...
def main() -> None:
    ...
    summary: dict[str, Any] = {
        ...
        "notes": [
            (
                "Probability-stable MSP requires confidence on the correct label "
                "to stay above threshold for all later steps."
            ),
    ...
    for threshold in THRESHOLDS:
        stats = cast(dict[str, Any], cast(dict[str, Any], summary["by_threshold"])[str(threshold)])
        overall = cast(dict[str, Any], stats["overall"])
        lines += [
            f"## Threshold `{threshold}`",
            (
                f"- coverage: `{overall['tasks_with_msp']}/{overall['tasks']}` "
                f"({overall['coverage']:.3f})"
            ),
        ...
        for benchmark, s in cast(dict[str, dict[str, Any]], stats["by_benchmark"]).items():
            lines.append(
                (
                    f"- `{benchmark}`: coverage=`{s['tasks_with_msp']}/{s['tasks']}` "
                    f"({s['coverage']:.3f}), mean_msp=`{s['mean_msp']}`, "
                    f"dist=`{s['distribution']}`"
                )
            )
        lines += ["", "### By Small Family"]
        for family, s in cast(dict[str, dict[str, Any]], stats["by_small_family"]).items():
            lines.append(
                (
                    f"- `{family}`: coverage=`{s['tasks_with_msp']}/{s['tasks']}` "
                    f"({s['coverage']:.3f}), mean_msp=`{s['mean_msp']}`, "
                    f"dist=`{s['distribution']}`"
                )
            )
