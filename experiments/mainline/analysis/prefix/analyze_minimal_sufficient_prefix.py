...
OUTPUT_MD = (
    ...
    / "analysis"
    / "prefix"
    / "minimal_sufficient_prefix_analysis.md"
)
...
def _build_matrix(
    rows: list[Row],
) -> tuple[list[Row], list[Row], np.ndarray, np.ndarray, np.ndarray, np.ndarray, list[str]]:
    train_rows: list[Row] = []
    test_rows: list[Row] = []
    for row in rows:
...
def _minimal_sufficient_step(sorted_rows: list[dict[str, Any]], label: int) -> int | None:
...
def _first_correct_step(sorted_rows: list[dict[str, Any]], label: int) -> int | None:
...
def _summarize(groups: list[TaskGroup]) -> dict[str, Any]:
    total = len(groups)
...
def main() -> None:
    ...
    by_small_family: dict[str, dict[str, Any]] = {}
    ...
    summary: dict[str, Any] = {
        ...
        "notes": [
            (
                "Minimal sufficient prefix is defined as the earliest step whose "
                "prediction matches the true label and remains unchanged for all "
                "later steps."
            ),
            (
                "This first version uses a discrete prediction trajectory with a "
                "single global threshold chosen on the train split."
            ),
            (
                "Large-family features are intentionally excluded because the "
                "target is small full-trace success, not takeover value."
            ),
        ],
    }
    ...
    lines = [
        ...
        (
            f"- tasks with MSP: `{overall['tasks_with_msp']}` / "
            f"`{overall['tasks']}` ({overall['msp_coverage']:.3f})"
        ),
    ...
    for benchmark, stats in by_benchmark.items():
        lines += [
            f"### `{benchmark}`",
            (
                f"- coverage: `{stats['tasks_with_msp']}/{stats['tasks']}` "
                f"({stats['msp_coverage']:.3f})"
            ),
    ...
    for row in representative_early[:8]:
        lines.append(
            (
                ...
                f"msp=`{row['minimal_sufficient_step']}`, "
                f"prefixes=`{row['num_prefixes']}`, label=`{row['label']}`"
            )
        )
