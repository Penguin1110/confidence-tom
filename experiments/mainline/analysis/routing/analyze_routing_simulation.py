...
class OracleStepMeta(TypedDict):
    prefix_id: str
    large_takeover_correct: int
    small_continue_correct: int
    delta_correctness: float
    large_total_tokens: int | float | None
    small_total_tokens: int | float | None


class OracleTaskMeta(TypedDict):
    benchmark: str
    small_family: str
    large_family: str
    full_trace_correct: int
    steps: dict[int, OracleStepMeta]


class SplitMatrix(TypedDict):
    train_rows: list[Row]
    test_rows: list[Row]
    x_train: np.ndarray
    y_train: np.ndarray
...
class PolicyRecord(TypedDict):
    ...
    small_family: str
    large_family: str
    baseline_small_correct: int
    route_used: int
    trigger_step: int | None
    policy_final_correct: int
    selected_large_correct: int | None
...
def load_oracle_index() -> dict[tuple[str, str], OracleTaskMeta]:
    index: dict[tuple[str, str], OracleTaskMeta] = {}
    for run_name, benchmark, small_family, large_family in RUN_SPECS:
        data = cast(
            list[dict[str, Any]],
            json.loads(find_result_json(run_name).read_text(encoding="utf-8")),
        )
        for task in data:
            steps: dict[int, OracleStepMeta] = {}
            for step in cast(list[dict[str, Any]], task["prefix_oracle_steps"]):
...
def build_matrix(rows: list[Row]) -> SplitMatrix:
...
def simulate_policy(
    *,
    ...
    oracle_index: dict[tuple[str, str], OracleTaskMeta],
    max_step: int,
    ...
    for key, rows in task_rows.items():
        ...
        # Oracle comparator under same max_step: route iff some positive step
        # exists within max_step.
        oracle_choice = None
    ...
    baseline_accuracy = avg([float(oracle_index[key]["full_trace_correct"]) for key in task_rows])
...
def summarize_by_group(sim: dict[str, Any], group_key: str) -> dict[str, Any]:
    buckets: dict[str, list[PolicyRecord]] = {}
    for row in cast(list[PolicyRecord], sim["records"]):
        buckets.setdefault(str(cast(dict[str, Any], row)[group_key]), []).append(row)
    out: dict[str, Any] = {}
...
def main() -> None:
    ...
    test_task_rows: dict[tuple[str, str], list[dict[str, Any]]] = {}
    ...
    def _sort_key(cfg: dict[str, Any]) -> tuple[float, float]:
        gain = (
            float(cfg["policy_gain_over_small"])
            if cfg["policy_gain_over_small"] is not None
            else float("-inf")
        )
        oracle_gap = (
            -float(cfg["gap_to_oracle"]) if cfg["gap_to_oracle"] is not None else float("-inf")
        )
        return (gain, oracle_gap)
    ...
    output: dict[str, Any] = {
        "notes": [
            (
                "Policy uses the Early Decision state_plus_family predictor "
                "trained on training tasks only."
            ),
            (
                "For each test task, route at the earliest prefix with failure "
                "probability >= threshold and step <= max_step."
            ),
            (
                "If routed, final correctness is the observed large takeover "
                "correctness at that prefix; otherwise the task keeps the "
                "small full-trace outcome."
            ),
            (
                "Oracle comparator uses the same max_step budget and routes "
                "only when a positive takeover exists within budget."
            ),
        ],
    ...
    lines = [
        ...
        "",
        (
            "這份模擬把 Early Decision predictor 直接接到 prefix-level "
            "routing policy：當 `P(small failure)` 超過門檻時，就在最早可用 "
            "prefix 交給 large takeover。"
        ),
        "",
        ...
        "",
        (
            "| max_step | fail threshold | small acc | policy acc | gain | "
            "oracle acc | gap to oracle | route rate | mean trigger step |"
        ),
        "| ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]

    for cfg in cast(list[dict[str, Any]], output["top_configs_by_gain"]):
        lines.append(
            (
                "| {max_step} | {failure_threshold:.1f} | "
                "{baseline_small_accuracy:.3f} | {policy_accuracy:.3f} | "
                "{policy_gain_over_small:.3f} | "
                "{oracle_accuracy_same_budget:.3f} | {gap_to_oracle:.3f} | "
                "{route_rate:.3f} | {mean_trigger_step_val} |"
            ).format(
    ...
    best = cast(list[dict[str, Any]], output["top_configs_by_gain"])[0]
    lines.extend(
        [
            "",
            ...
            "",
            (
                f"- best config: `max_step={best['max_step']}`, "
                f"`failure_threshold={best['failure_threshold']}`"
            ),
    ...
    for name, stats in cast(dict[str, dict[str, Any]], best["by_benchmark"]).items():
        lines.extend(
            [
                f"#### {name}",
                "",
