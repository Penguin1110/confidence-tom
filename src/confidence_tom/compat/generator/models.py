...
class ConfidenceJustification(BaseModel):
    ...
    uncertainty_factors: List[str] = Field(
        description=(
            "Factors that reduce confidence (e.g., ambiguous wording, multiple interpretations)."
        )
    )
    confidence_anchors: List[str] = Field(
        description=(
            "Factors that increase confidence (e.g., clear logical chain, verified calculation)."
        )
    )
...
class SubjectOutputV2(BaseModel):
    ...
    task_type: str = Field(
        default="unknown", description="Task type (e.g., 'navigate', 'formal_fallacies')"
    )
    ...
    behavioral_confidence: float = Field(
        description=(
            "c_beh: fraction of samples with CORRECT answer (0.0 - 1.0). This is the Ground Truth."
        )
    )
    consistency_rate: float = Field(
        default=0.0,
        description="Fraction of samples matching majority_answer (self-consistency metric).",
    )
