...
class ApiTrace(BaseModel):
    ...
    reasoning_tokens: int = Field(
        default=0, description="Tokens used for internal reasoning (thinking)"
    )
    reasoning_content: str = Field(
        default="", description="Reasoning text if exposed by model (e.g. DeepSeek-R1)"
    )
...
class TrajectoryStep(BaseModel):
    ...
    step_confidence: int = Field(
        ge=0, le=100, description="Agent's confidence at this step (0-100)"
    )
...
class DynamicTask(BaseModel):
    ...
    benchmark: str = Field(
        description="Source benchmark: 'tau-bench', 'bird-sql', 'plancraft', 'intercode'"
    )
    ...
    metadata: dict[str, Any] = Field(
        ...
        description="Benchmark-specific extra data",
    )
...
class AgentRun(BaseModel):
    ...
    is_correct: bool = Field(
        description="Whether the benchmark evaluator deemed this run successful"
    )
    ...
    api_trace: Optional[ApiTrace] = Field(
        default=None, description="Raw API metadata for this call"
    )
...
class TaskResult(BaseModel):
    ...
    behavioral_confidence: float = Field(description="C_beh: fraction of runs that succeeded (0-1)")
