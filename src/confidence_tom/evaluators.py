...
def _tau_action_supported(action: dict, haystack: str) -> bool:
    ...
    matched_values = sum(1 for token in salient_values if _normalize_output_token(token) in haystack)
...
def _parse_plancraft_inventory(instruction: str) -> dict[str, int]:
    match = re.search(r"Your current inventory:\s*(.+?)\.\s*List the crafting steps", instruction, re.DOTALL)
    if not match:
        return {}
